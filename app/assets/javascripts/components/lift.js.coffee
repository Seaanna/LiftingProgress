...
@Lift = React.createClass
    ...
    handleDelete: (e) ->
        ...
        $.ajax
            ...
            dataType: 'JSON'
            success: () =>
                # notifying the parent component Lifts by sending the component lift data to the prop handleDeleteLift
                @props.handleDeleteLift @props.lift
    handleEdit: (e) ->
        e.preventDefault()
        # taking the values from the forms using refs
        data =
        ...
        $.ajax
            ...
            data:
                lift: data
            #on success,we set state.edit to false and communicate witht the parent to update UI
            success: (data) =>
                @setState edit: false
    ...
    getOneRm: (weight, reps) ->
        if weight and reps > 0 and reps < 11
            ...
            0
    toggleUnit: (e) ->
        # update button should trigger handleEdit
        e.preventDefault()
        @setState ismetric: !@state.ismetric
    # liftrow will return the current row with the edit button added
    liftRow: ->
        React.DOM.tr null,
